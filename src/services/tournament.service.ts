...
const cronService = new CronService(); // Or use dependency injection
...
class TournamentService {
    ...
    try {
      const tournament = await prisma.tournament.create({
        data: {
          ...
          match_interval: parseInt(match_interval || '30'),
      ...
      if (prizePools && prizePools.length > 0) {
        ...
        // Get all users who favorited the game and create a notification for each user
        const favUserIds = await prisma.game_favaurites.findMany({
          where: {
            game_id: parseInt(games_id),
          },
          select: {user_id: true},
        });

        if (favUserIds && favUserIds.length > 0) {
          // Create notifications for each favorited user
          for (const fav of favUserIds) {
            const noti = await prisma.notification.create({
              data: {
                userId: fav.user_id!,
                message: `A new tournament "${tournament_name}" for your favorite game has been created.`,
            ...
            const socketId = await getSocketIdByUserId(fav.user_id!.toString());
            if (socketId) {
  ...
  async getTournaments() {
    // sort by tournament_registration_start_date do not filter just sort
    const tournaments = await prisma.tournament.findMany({
      orderBy: {
        tournament_registration_start_date: 'asc',
      },
      include: {
        games: true,
      },
  ...
  async registerTournament(user_id: string, tournament_id: string) {
    ...
    console.log("🚀 ~ TournamentService ~ registerTournament ~ tournament:", tournament)
    if (!tournament) throw HttpException.notFound('Tournament not found.');
    const entryFee = Math.round(tournament.tournament_entry_fee ?? 0);  // make it an integer
    if (entryFee > 0) {
      // 1) load the user to check balance
      const user = await prisma.user.findUnique({
        where: { id: Number(user_id) },
        select: { clutch_bucks: true },
      });
      if (!user) throw HttpException.notFound('User not found.');
      if (user.clutch_bucks < entryFee) {
  ...
  private async initializePointsBasedMatches(tournamentId: string, totalRounds: number) {
    ...
    const matchIntervalMs = tournament.match_interval * 60 * 1000; // Convert minutes to milliseconds

    for (let round = 1; round <= totalRounds; round++) {
      // Calculate match time for this round
      const roundTime = new Date(
        tournament.tournament_start_date.getTime() + (round - 1) * matchIntervalMs,
      );
      ...
      tournament.participants.forEach((participant) => {
        const matchData: any = {
          ...
          match_time: roundTime, // Use calculated round time
        };
        if (isTeamTournament) {
  ...
  private async initializeMatches(tournamentId: string) {
    try {
      ...
      for (let round = 1; round <= numberOfRounds; round++) {
        ...
        const roundMatchTime = new Date(
          tournamentStartDate.getTime() + (round - 1) * matchInterval * 60 * 1000,
        );
        ...
        if (round === 1) {
          for (let i = 0; i < players.length; i += 2) {
            matchesData.push({
              tournamentId: Number(tournamentId),
              match_time: roundMatchTime,
              round: round,
              position: i / 2, 
              player1Id: players[i],
              player2Id: players[i + 1] || null,
            });
            console.log('🚀 ~ TournamentService ~ initializeMatches ~ matchesData:', matchesData);
          }
        } else {
      ...
      // Fetch created matches to schedule reminders
      const createdMatches = await prisma.match.findMany({
        where: {tournamentId: Number(tournamentId)},
        select: {id: true, match_time: true},
      });
      console.log('🚀 ~ TournamentService ~ initializeMatches ~ createdMatches:', createdMatches);

      createdMatches.forEach((match) => {
        cronService.scheduleReminderForMatch(match.id, match.match_time!);
      });

      console.log('Matches initialized with reminders scheduled.');
    } catch (error) {
          ...
          if (isPointsBased) {
            ...
            if (submission.isTeam) {
              ...
              const inTeam1 = match.team1?.teamPlayers.some(
                (tp) => tp.user_id === submission.submittedBy,
              );
