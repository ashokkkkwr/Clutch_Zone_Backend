  ...
  daysInWeek.push({
    ...
    users: 0,
          ...
          const userCounts = await prisma.$queryRaw<{ day: Date, users: number }[]>`
            ...
            ORDER BY day
          `;
          console.log("🚀 ~ Dashboard ~ getAdminDashboardStats ~ userCounts:", userCounts)
...
const weeklyUsers = daysInWeek.map((day) => {
  ...
  return {
    ...
    users: userCountsMap.get(dayKey) || 0,
          ...
          console.log("🚀 ~ Dashboard ~ weeklyUsers ~ weeklyUsers:", weeklyUsers)
          ...
          const stats = {
            ...
            matches: { total: totalMatches, scoreSubmissionStatusDistribution: scoreSubmissionStatusStats },
